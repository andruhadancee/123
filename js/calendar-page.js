...
(function(){
    ...
    function isTournamentActive(event) {
        ...
        try {
            ...
            const timeMatch = event.start_time.match(/(\d{1,2}):(\d{2})(?::\d{2})?/);
        ...
        for(let day=1; day<=daysInMonth; day++){
            ...
            if (dayEventsFiltered.length > 0){
                ...
                if (uniqueDisciplines.length === 1) {
                    ...
                    // Создаём градиентную обводку через wrapper
                    const gradientWrapper = document.createElement('div');
                    gradientWrapper.style.cssText = `position: absolute; top: 0; left: 0; right: 0; bottom: 0; border-radius: 8px; padding: 2px; background: linear-gradient(to right, ${color1} 0%, ${color1} 50%, ${color2} 50%, ${color2} 100%); pointer-events: none; z-index: 0;`;
                    const gradientInner = document.createElement('div');
                    gradientInner.style.cssText = `width: 100%; height: 100%; border-radius: 6px; background: ${activeBg}; pointer-events: none;`;
                    gradientWrapper.appendChild(gradientInner);
                    cell.appendChild(gradientWrapper);
                    // Убираем стандартную border
                    ...
                    ${uniqueEvents.map(e => {
                        ...
                        <div class="calendar-event-card">
                            ...
                            <div class="calendar-event-content">
                                ...
                                ${e.start_time ? `<div class="calendar-event-time">Время старта: ${e.start_time.split(':').slice(0, 2).join(':')} МСК</div>` : ''}
